...
<h2>All New Orders</h2>
   <p style="amountAssigned"> <strong>Total Amount </strong>: <?php echo $totalprice; ?> KD</p>
        ...
        if(!empty($customer_orders)){
           foreach($customer_orders as $k=>$v){
            ...
            <tr>        
                ...
                <td>
                    <a class="button tips view" title="View Details" href="<?php echo CUSTOM_ADMIN_URL.'post.php?post='.$v->ID.'&action=edit';?>"><i class="fa fa-eye"></i></a>
                     <a class="button tips view" title="Edit Details" href="<?php echo CUSTOM_ADMIN_URL.'post.php?post='.$v->ID.'&action=edit';?>"><i class="fa fa-pencil"></i></a>
                </td>           
            </tr> 
        <?php
          $counter++;
